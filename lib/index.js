...
function parseProject(solidityCompilerPath, solidityCompilerExtraArgs, contractsPath) {
  var files = _shelljs2.default.find(contractsPath).filter(function(file) { return file.match(/\.sol$/); }).join(' ');
  var commandOutput = _shelljs2.default.exec(['' + solidityCompilerPath, '  --pretty-json', '  --allow-paths ' + contractsPath, '  --combined-json ' + COMBINED_JSON_OPTIONS.join(','), '  ' + solidityCompilerExtraArgs, '  ' + files].join(' '), { silent: true });
